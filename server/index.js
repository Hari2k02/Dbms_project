...
app.get('/items', async (req, res) => {
    ...
    if (item_name) {
        allItemsQuery = `
        select i.item_id ,i.item_name, i.price, sum(coalesce(oi.qty_received, 0)) as qty_on_hand, sum(coalesce(oi.qty_ordered, 0)) as qty_on_order
        from item i
        left join orders_item oi on
        oi.item_id = i.item_id
        where i.item_name ilike '%${item_name}%'
...
app.post('/items', async (req, res) => {
    try {
        ...
        returning *;
        `
        const newItem = await pool.query(newItemQuery, [item_name,price]);
...
app.post('/brands', async(req,res)=>{
    try {
        ...
        const newBrandQuery = `insert into brands(brand_name,status) values($1,$2) returning *;`
        const newBrand = await pool.query(newBrandQuery,[brand_name,status]);
...
//delete a brand
app.delete('/brands', async (req, res) => {
    try {
        const { checkedBrands } = req.body;
        // deleting a brand from brand table will automatically delete the rows in order item table with that same item id
        const deleteBrandsQuery = `delete from brands where brand_id = $1;`
        ...
        for (let brandId of checkedBrands) {
            await pool.query(deleteBrandsQuery, [brandId]);
        }
...
app.post('/categories', async(req,res)=>{
    try {
        ...
        const newCatQuery = `insert into category(cat_name,status) values($1,$2) returning *;`
        const newCat = await pool.query(newCatQuery,[cat_name,status]);
...
app.delete('/categories', async (req, res) => {
    try {
        const { checkedCats } = req.body;
        // deleting a brand from brand table will automatically delete the rows in order item table with that same item id
        const deleteCatsQuery = `delete from category where cat_id = $1;`
        // to clean up the orders that have no record in the order item table anymore
        const deleteOrdersQuery = `delete from orders o where order_id not in (select distinct order_id from orders_item);`
        for (let catId of checkedCats) {
            await pool.query(deleteCatsQuery, [catId]);
        }
        await pool.query(deleteOrdersQuery);
        res.json();
    } catch (error) {
...
app.get('/orders/:orderId', async (req, res) => {
    try {
        ...
        select o.ref_number, i.item_id, i.item_name, i.price, oi.qty_received, oi.qty_ordered
        from orders_item oi
        inner join item i on
            i.item_id = oi.item_id
        inner join orders o on
