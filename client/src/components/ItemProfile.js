...
const ItemProfile = ({ match }) => {
    ...
    const getItem = async () => {
        try {
            ...
            setInitialDescription(item.item_name);
            setPrice(item.price);
        } catch (error) {
            console.log(error.message);
        }
    ...
    const validateDescription = () => {
        ...
        if (!(item_name.length > 0) && !(item_name.length <= 50)) {
            setDescriptionErr(true);
            setDescriptionHelperText('Must be 1-50 characters long');
            return;
        }

        if (!(/^[a-zA-Z\s]*$/).test(item_name)) {
            setDescriptionErr(true);
        ...
        for (let item of items) {
            if (item_name !== initialDescription && item.item_name === item_name) {
                setDescriptionErr(true);
